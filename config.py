...
def main() -> None:
    """Demonstrate how to use the configuration values."""

    print("Step counter goals:")
    print(f"  Default: {StepCounter.default_goal} steps")
    print(f"  Minimum: {StepCounter.minimum_goal} steps")
    print(f"  Maximum: {StepCounter.maximum_goal} steps")

    print("\nLevel energy by tier:")
    for tier in ItemTier:
        print(f"  {tier.value.title()}: {Pet.LevelEnergy.energy(tier)} energy")

    print("\nSample store prices:")
    for category in ItemCategory:
        basic_price = Pet.StorePrice.price(category, ItemTier.BASIC)
        premium_price = Pet.StorePrice.price(category, ItemTier.PREMIUM)
        print(
            f"  {category.value.title()}: basic {basic_price} coins, "
            f"premium {premium_price} coins"
        )

    print("\nStatus decay interval:")
    print(f"  Every {StatusDecay.interval} seconds")

    print("\nNatural growth experience:")
    print(f"  Per day: {NaturalGrowth.DaysExperience.per_day}")
    print(f"  Per feeding: {NaturalGrowth.FeedingExperience.per_feeding}")
